...
import { columnsTableDriver as columns } from 'src/constants/constants';
...
import ModalDriver from 'src/components/Modal/Driver.vue';

const emit = defineEmits<TableEmits>();
const props = withDefaults(defineProps<TableProps<Driver[]>>(), {
  loading: false,
  list: () => [],
  hiddenFields: () => [],
  pagination: () => ({
...
const getDrivers = async () => {
  ...
  if (mode.value === ModeTable.All) {
    ...
    $API.getAllDrivers(
      async (response: ResponseType<Driver[]>) => {
...
<template>
  ...
  <ModalDriver v-if="modal" v-model="modal" :chat-id="selectedChatId" />
</template>
